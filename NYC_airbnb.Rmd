...
# set.seed(777)
# 
# knn.fit <- train(log_price ~., data = data_train, 
#                  method = "knn", 
#                  trControl = fitControl,
#                  tuneGrid = expand.grid(k = seq(10,100,by=5)))
# 
...
rcvCtrl <- trainControl(method = "repeatedcv", 
                        number = 5, repeats = 3)

Grid <- expand.grid(n.trees = c(100, 125, 150, 175), 
...
getModelInfo()$gbm_fit$parameters
...
set.seed(777)
rf_fit  <-  train(log_price~., data = data_train,
                   method = "rf",
                   trControl = trctrl)
...
rf_pred <- predict(rf_fit, newdata = data_model)
...
```{r cache=TRUE,include=T,message=FALSE, warning=FALSE, echo=F}
#linear kernel
svm_fit = svm(log_price ~., data = data_train , type="nu-regression",
              kernel="linear", shrinking = T )

svm_pred = predict(svm_fit, newdata = data_test)

#RMSE for linear SVM
mean((svm_pred - data_test$log_price))
```
### 0.5

```{r cache=TRUE,include=T,message=FALSE, warning=FALSE, echo=F}
#polynomial kernel
svm_fit2 = svm(log_price ~., data =data_train , type="nu-regression",
         kernel="polynomial", shrinking = T )

svm_pred2 = predict(svm_fit2, newdata = data_test)

#RMSE for polynomial SVM
mean((svm_pred2 -data_test$log_price))
```
### 0.56
