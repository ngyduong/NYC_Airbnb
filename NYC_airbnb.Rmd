...
saveRDS(rf_fit, "/Users/ngyduong/Documents/Machine Learning/Github_projects/NewYork_Airbnb/NYC_Airbnb/rf_fit.rds")
```

```{r cache=TRUE,include=T,message=FALSE, warning=FALSE, echo=T}
rf_fit <- readRDS("/Users/ngyduong/Documents/Machine Learning/Github_projects/NewYork_Airbnb/NYC_Airbnb/rf_fit.rds")
rf_pred <- predict(rf_fit, newdata = data_test)
```
...
df$Catégorie <- factor(ifelse(df$model %in% c("KNN", "Boosting", "XGBoost", 
                                             "RForest"), 1, 0), 
                      labels = c("Linéaire", "Non Linéaire"))
...
ggplot(data = df, aes(x = fct_reorder(model, RMSE, .desc = T), 
  ...
  ggtitle("Performances des modèles par RMSE (basé sur le Train set)") +
  scale_fill_manual(values=c("blue3", "green3"))

# Rsquared
ggplot(data = df, aes(x = fct_reorder(model, Rsquared, .desc = F), 
                      y = Rsquared, fill = Catégorie)) +
  geom_boxplot(outlier.colour = "darkblue", outlier.size = 2, alpha=0.2) +
  theme_models2 + 
  labs(x = "", y = "Rsquared", 
       caption = "All the models were fit using the Train set with 62762 observations and 21 variables. 
       The validation methode chosen was a 5 Fold Cross Validation") +
  ggtitle("Performances des modèles par Rcarré (basé sur le Train set)") +
  scale_fill_manual(values=c("blue3", "green3"))
```
...
ggplot(data = df, aes(x = fct_reorder(model, RMSE, .desc = T), 
  ...
  labs(x = "", y = "RMSE",
       ...
       The validation methode chosen was a 5 Fold Cross Validation") +
  geom_text(aes(label = round(RMSE, digits = 2)), size = 3,
            position = position_dodge(0.9), vjust = -0.5) +
  ggtitle("Performances des modèles par RMSE (basé sur le Test set)") +
  scale_fill_manual(values=c("blue3", "green3"))
...
ggplot(data = df, aes(x = fct_reorder(model, Rsquared, .desc = F), 
  ...
  labs(x = "", y = "Rsquared", 
       caption = "All the models were fit using the Test set with 15689 observations and 21 variables.
       The validation methode chosen was a 5 Fold Cross Validation") +
  geom_text(aes(label = round(Rsquared, digits = 2)), size = 3,
            position = position_dodge(0.9), vjust = -0.5) +
  ggtitle("Performances des modèles par Rcarré (basé sur le Test set)") +
  scale_fill_manual(values=c("blue3", "green3"))
```
...
difference = data.frame(real_price = exp(data_test$log_price))
for (i in pred_list){
  x <- as.data.frame(abs(exp(get(i)) - exp(data_test$log_price)))
  names(x) <- paste(substr(i,1,nchar(i)-5),"diff", sep="_")
  difference <- cbind(difference, x)
}

summary_difference = data.frame()
for (i in names(difference[,-c(1)])){
  summary_difference <- rbind(summary_difference, summary(difference[,i]))
}

summary_difference <- round(summary_difference,2)

names(summary_difference) <- c("Min", "1st Qu.", "Median", 
                               "Mean", "3rd Qu.", "Max")
summary_difference$model <- models_list
summary_difference$diff_prix_median <-
  paste("$",as.character(summary_difference$Median,2),sep="")

ggplot(data = summary_difference, 
       aes(x = fct_reorder(model, Median, .desc = T), y = Median, fill = model)) +
  geom_bar(stat = "identity", alpha=0.5) + theme_models +
  labs(x = "", y = "Différence médiane avec les vrais prix", 
       caption = "All the models were fit using the Test set with 15689 observations and 21 variables.
       The validation methode chosen was a 5 Fold Cross Validation") +
  geom_text(aes(label = diff_prix_median, vjust = -0.3)) +
  ggtitle("Différence en médiane entre les prédictions et la réalité") 
```
