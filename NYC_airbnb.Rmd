...
summary_difference$diff_prix <-
  paste("$",as.character(round(summary_difference$Mean,2)),sep="")

ggplot(data = summary_difference, 
       aes(x = fct_reorder(model, Mean, .desc = T), y = Mean, fill = model)) +
  geom_bar(stat = "identity", alpha=0.5) + theme_models +
  labs(x = "", y = "Différence moyenne avec les vrais prix", 
       caption = "All the models were fit using the Test set with 15689 observations and 21 variables.
       The validation methode chosen was a 5 Fold Cross Validation") +
  geom_text(aes(label = diff_prix, vjust = -0.3)) +
  ggtitle("Différence en moyenne entre les prédictions et la réalité") 
```
