...
knn_fit <- train(log_price ~., data = data_train,
                 method = "knn",
                 preProc = c("center","scale"),
                 trControl = fitControl,
                 tuneGrid = expand.grid(k = seq(8,22,by=2)))
...
knn_pred <- predict(knn_fit, data_test)

#RMSE for KNN
postResample(pred = knn_pred, obs = data_test$log_price)
```
...
```{r Setseeds, include=FALSE,echo=FALSE}
...
rcvCtrl <- trainControl(method = "cv", number = 5, classProbs = TRUE, 
                        savePredictions = TRUE, seeds = rcvSeeds)

Grid <- expand.grid(n.trees = 150:200, 
                    interaction.depth = 2,
                    shrinkage = .1, 
                    n.minobsinnode= 3)

set.seed(777)
gbm_fit  <-  train(log_price~., data = data_train,  
                  method = "gbm", verbose= FALSE,
                  bag.fraction = 1, 
                  distribution = "gaussian",
                  trControl = rcvCtrl, tuneGrid = Grid)
plot(gbm_fit)              
gbm_fit$bestTune
...
rcvCtrl <- trainControl(method = "cv", number = 5, allowParallel = TRUE, 
                        verboseIter = FALSE, 
                        returnData = FALSE,
                        seeds = rcvSeeds)

Grid <- expand.grid(nrounds = 1:200,
                      max_depth = 5,
                      eta = 0.3,
                      gamma = 0.01,
...
set.seed(777)
xgboost_fit  <-  train(log_price~., data = data_train,  
                  method = "xgbTree", 
                  trControl = rcvCtrl, tuneGrid = Grid)
plot(xgboost_fit)              
...
```{r cache=TRUE,include=T,message=FALSE, warning=FALSE, echo=F}
models <- list(KNN = knn_fit, Boosing = gbm_fit, XCBoost = xgboost_fit)
res <- resamples(models)
colvec <- c("orange","darkolivegreen3", "blue","brown","yellow")
boxplot(res$values[c("Logit~Accuracy","KNN~Accuracy","RandomForest~Accuracy",
                     "BoostingTree~Accuracy")], 
